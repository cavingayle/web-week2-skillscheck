...
middleNums = numbers.filter( num => num !== numbers[0] && num !== numbers[numbers.length - 1]) 
//////////////////PROBLEM 11////////////////////
...
const me = {
	...
	age: 27,
	greeter: function () {
		 return `Hello! My name ${name} and I live in ${state}`
	}
...
// Then, loop over the passed in arr parameter, and check to see if the number in the array is GREATER than 100. If it is, push 'big' as a string to the answers array. 
...
//loop over arr
// if number is bigger than 100 push 'big' to answers if less push small

function bigOrSmall(arr) {
	let answers = arr.map(num => {
		if (num <= 100) {
			return num = 'small'
		} else {
			return num = 'big'
...
const total = myNumbers.reduce((a,c) => a + c)
//////////////////PROBLEM 17////////////////////

// Finally, use .forEach to find the index of each item in the array. 
// To begin, create an empty array called 'myNumbersIndex'. Then, use forEach to push each item's index from the myNumbers array into the newly created myNumbersIndex array. 


let myNumbersIndex =[]
myNumbers.forEach((num, i) => myNumbersIndex.push(i) )
...
const forTheLoveOfGeorge = notGeorge.map(name => 'George' )
...
let totallyAwesome = people.reduce((a, c) => a + c.awesomeLevel, 0)
